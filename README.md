# Responsive Image Search App (React)

A minimalist and fully responsive Image Search Application built with React.js and the Pixabay REST API.  
The project uses pure CSS for styling (no Tailwind or UI libraries) and focuses on clean code and real responsiveness.

---

## Features
- Search images by keyword
- Fetches data from Pixabay API
- Fully responsive (mobile, tablet, desktop)
- Clean white minimalist UI
- Search via button or Enter key
- Simple and readable code structure

---

## Tech Stack
- React.js
- Axios
- Pixabay REST API
- Pure CSS (Flexbox + CSS Grid)

---

## How It Works
1. User enters a search term
2. App sends a request to Pixabay API
3. Images are fetched asynchronously
4. Results are displayed in a responsive grid
5. Each image shows basic statistics below it

